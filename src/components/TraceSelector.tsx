...
export const ALL_TRACES=CHANNELS.map(channel=>channel.id);
...
const groups=[
 {name:'Surface ECG',ids:['II','V1']},
 {name:'High right atrium',ids:['HRA']},
 {name:'His bundle',ids:['HISp','HISd']},
 {name:'Coronary sinus',ids:['CSp','CSm','CSd']},
 {name:'Right ventricle',ids:['RV']},
];
export function TraceSelector({selected,onChange}:{selected:string[];onChange:(ids:string[])=>void}){
 ...
 return <div className="trace-selector">
  <p>Choose the signals shown in the live recording and frozen review. All nine signals continue to be recorded.</p>
  ...
  <div className="trace-groups">{groups.map(group=><fieldset key={group.name}><legend>{group.name}</legend>{CHANNELS.filter(c=>group.ids.includes(c.id)).map(channel=><label key={channel.id}>

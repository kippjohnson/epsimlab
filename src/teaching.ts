import {ADVANCED_TEACHING} from './advanced-teaching.ts';
...
export const DIAGNOSES=[
 ...
 ...Object.entries(ADVANCED_TEACHING).map(([id,t])=>({id,label:t.name,sub:'Advanced diagnostic study'})),
...
export function observations(events:LabEvent[]){
 ...
 const marker=(id:string)=>events.some(e=>e.evidence===id);
 const pvcResponses=events.filter(e=>e.evidence==='hisPVC').flatMap(pvc=>{
  const prior=events.filter(e=>e.kind==='activation'&&e.node==='A'&&e.t<pvc.t).slice(-5),steps=intervals(prior);
  const next=events.find(e=>e.kind==='activation'&&e.node==='A'&&e.t>pvc.t);
  const confounded=next&&events.some(e=>e.t>pvc.t&&e.t<next.t&&['stimulus','shock'].includes(e.kind));
  return next&&!confounded&&steps.length>=3&&spread(steps)<5?[next.t-prior.at(-1)!.t-mean(steps)]:[];
 });
 const facts=[
  {id:'pvcReset',label:'A His-refractory PVC changes the predicted next atrial activation',valid:pvcResponses.some(x=>Math.abs(x)>10),why:'Confirm stable baseline A–A, ventricular capture and His refractoriness, then measure the atrial timing change.'},
  {id:'pvcUnchanged',label:'A His-refractory PVC leaves the predicted next atrial activation unchanged',valid:pvcResponses.some(x=>Math.abs(x)<=10),why:'A captured, appropriately timed PVC must follow a stable atrial baseline. This negative response alone does not exclude an accessory pathway.'},
  ...['hisPVC','infraHis','preexcitation','mapCollected'].map(id=>({id,label:({hisPVC:'A captured PVC delivered during His refractoriness',infraHis:'His activation with downstream conduction block',preexcitation:'Ventricular activation through the antegrade accessory route',mapCollected:'A complete local activation mapping point collected'} as Record<string,string>)[id],valid:marker(id),why:'Review the relevant captured stimulus or complete local recording; a selected tool alone is not evidence.'})),
  {id:'pacedEvidence',label:'A pacing protocol with confirmed capture',valid:events.filter(e=>e.kind==='stimulus'&&e.captured).length>=3,why:'Check local capture after each stimulus.'},
  {id:'prolongedHV',label:'Consistently prolonged antegrade HV',valid:recent.filter(e=>e.node==='V').filter(e=>{let h=byId.get(e.cause??-1);return h?.node==='H'&&e.t-h.t>=70;}).length>=3,why:'Measure H to V in baseline antegrade beats.'},
  {id:'avDissociation',label:'Atrial and ventricular cycles differ during ventricular tachycardia',valid:aa.length>=3&&vv.length>=6&&Math.abs(mean(aa)-mean(vv))>100&&v.some(e=>['rvot','fascicular','scar'].includes(e.morphology??'')),why:'Follow A and V separately outside a pacing train.'},
...
export const GLOSSARY=[
 ...
 ['S1 / S2 / S3','S1 is a series of evenly spaced paced beats. S2 follows the last S1 at a chosen coupling interval; S3 follows S2. Extra beats test tissue recovery and conduction.'],
 ['Entrainment / PPI','Entrainment means pacing temporarily drives a reentrant rhythm. Confirm chamber acceleration and capture first. PPI is the time from the last stimulus to the returning local activation; compare it with the baseline cycle using the same recording site.'],
 ['His-refractory PVC','A premature ventricular beat delivered while the His bundle is still recovering. Confirm ventricular capture, then compare atrial timing and subsequent cycles. A negative response alone cannot exclude an accessory connection.'],
 ['Para-Hisian pacing','Compare retrograde atrial timing with His plus ventricular capture versus ventricular-only capture at the same pacing cycle. Accidental direct atrial capture invalidates the comparison.'],
 ['Activation / voltage / pace mapping','Activation mapping compares local signal timing with a reference. Voltage mapping measures local signal amplitude. Pace mapping compares paced surface morphology with the clinical rhythm. Each supplies a different piece of evidence.'],

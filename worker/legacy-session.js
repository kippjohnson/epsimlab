...
export default {
 async fetch(request,env){
  ...
  try{
   ...
   let previous=0;for(const c of s.commands){if(!c||!Number.isFinite(c.t)||c.t<previous||c.t>s.now||!validCommand(c,session.engineVersion))return json({error:'Invalid command history'},400);previous=c.t;}

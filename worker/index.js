...
export default {
 async fetch(request,env,ctx){
  ...
  try{
   if(path==='/api/health')return json({app:'epsimlab',version:'0.4.0',engineVersion:ENGINE_VERSION,accounts:configured(env),storage:configured(env)||Boolean(env.DB&&env.RECORDINGS)});
   ...
   const response=path==='/api/studies'?await studiesAPI(request,env):await accountsAPI(request,env,ctx);

...
export type EventKind = 'activation' | 'stimulus' | 'block' | 'note';
...
export type Command = {t:number; type:'pace'; protocol:Protocol}|{t:number;type:'stop'};
...
export const isCompatibleSession=(version:string,caseId:string)=>CASES.some(c=>c.id===caseId)&&(version===ENGINE_VERSION||version==='0.1.0'&&['avnrt-01','avnrt-02'].includes(caseId));

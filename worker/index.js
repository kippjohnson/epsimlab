...
export default {
 async fetch(request,env,ctx){
  ...
  try{
   if(path==='/api/health')return json({app:'epsimlab',version:'0.6.0',engineVersion:ENGINE_VERSION,accounts:configured(env),storage:configured(env)||Boolean(env.DB&&env.RECORDINGS)});
